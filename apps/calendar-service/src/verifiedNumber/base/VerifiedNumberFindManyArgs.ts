/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { VerifiedNumberWhereInput } from "./VerifiedNumberWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { VerifiedNumberOrderByInput } from "./VerifiedNumberOrderByInput";

@ArgsType()
class VerifiedNumberFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => VerifiedNumberWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => VerifiedNumberWhereInput, { nullable: true })
  @Type(() => VerifiedNumberWhereInput)
  where?: VerifiedNumberWhereInput;

  @ApiProperty({
    required: false,
    type: [VerifiedNumberOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [VerifiedNumberOrderByInput], { nullable: true })
  @Type(() => VerifiedNumberOrderByInput)
  orderBy?: Array<VerifiedNumberOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { VerifiedNumberFindManyArgs as VerifiedNumberFindManyArgs };
