/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AccessCodeWhereUniqueInput } from "./AccessCodeWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { AccessCodeUpdateInput } from "./AccessCodeUpdateInput";

@ArgsType()
class UpdateAccessCodeArgs {
  @ApiProperty({
    required: true,
    type: () => AccessCodeWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AccessCodeWhereUniqueInput)
  @Field(() => AccessCodeWhereUniqueInput, { nullable: false })
  where!: AccessCodeWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => AccessCodeUpdateInput,
  })
  @ValidateNested()
  @Type(() => AccessCodeUpdateInput)
  @Field(() => AccessCodeUpdateInput, { nullable: false })
  data!: AccessCodeUpdateInput;
}

export { UpdateAccessCodeArgs as UpdateAccessCodeArgs };
